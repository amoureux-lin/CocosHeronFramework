...
export class NetNode {
    ...
    protected onConnected(event: any) {
        ...
        console.net(`网络已连接当前状态为【${NetNodeStateStrs[this._state]}】`);
    }

    /** 连接验证成功，进入工作状态 */
    protected onChecked() {
        console.net("连接验证成功，进入工作状态");
        ...
        if (requests.length > 0) {
            console.net(`请求【${this._requests.length}】个待发送的信息`);
    ...
    protected onMessage(msg: any): void {
        ...
        if (this._requests.length > 0) {
            ...
            if (this._requests.length == 0) {
                ...
                console.net(`请求队列中还有【${this._requests.length}】个请求在等待`);
            }
        }
    ...
    private base_request(reqProtocol: IRequestProtocol, rspCmd: string, rspObject: CallbackObject, showTips: boolean = true, force: boolean = false) {
        ...
        console.net(`队列命令为【${rspCmd}】的请求，等待请求数据的回调`);
