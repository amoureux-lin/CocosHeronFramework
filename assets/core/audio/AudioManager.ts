...
export class AudioManager extends Component {
    ...
    /**
     * 播放背景音乐
     * @param url        声音名res/audio/【xx】
     * @param callback   音乐播放完成事件
     * @param bundleName 资源包名
     */
    playBundleMusic(name: string,loop:boolean = false, callback?: Function) {
        let url = 'res/audio/'+name;
        if (this.music.switch) {
            this.music.loop = loop;
            let bundleName = app.res.currentBundle;
            this.music.load(url, callback, bundleName).then();
        }
    }
     ...
     * @param bundleName 资源包名
     */
    playBundleEffect(name: string | AudioClip, bundleName?: string, onPlayComplete?: Function): Promise<number> {
        let url = 'res/audio/'+name;
        return this.effect.load(url, bundleName, onPlayComplete);
    }

    /** 回收音效播放器 */
    putBundleEffect(aeid: number, name: string | AudioClip, bundleName?: string) {
        let url = 'res/audio/'+name;
        this.effect.put(aeid, url, bundleName);
    }
    ...
    save() {
        ...
        app.storage.set(LOCAL_STORE_KEY, this.local_data);
    }

    /** 本地加载音乐音效的音量、开关配置数据并设置到游戏中 */
    load() {
        this.music = this.getComponent(AudioMusic) || this.addComponent(AudioMusic)!;
        this.local_data = app.storage.getJson(LOCAL_STORE_KEY);
        if (this.local_data) {
            try {
                this.setState();
            }
