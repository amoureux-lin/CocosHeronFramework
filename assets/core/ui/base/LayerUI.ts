...
import { UICallbacks,ViewParams } from "../UIDefine";
...
export class LayerUI extends Node {
    ...
    /**
     * 添加一个预制件节点到层容器中，该方法将返回一个唯一`uiName`来标识该操作节点
     * @param config     界面配置数据
     * @param params     自定义参数
     * @param callbacks  回调函数对象，可选
     * @returns ture为成功,false为失败
     */
    add(uiName: string, config: UIConfig, params?: any, callbacks?: UICallbacks) {
        if (this.ui_nodes.has(config.path)) {
            console.warn(`路径为【${config.path}】的预制重复加载`);
            return;
        }

        // 检查缓存中是否存界面
        let vp = this.ui_cache.get(config.path);
        if (vp == null) {
            vp = new ViewParams();
            vp.uiName = uiName;
            vp.config = config;
        }
        this.ui_nodes.set(config.path, vp);
    ...
    protected async load(vp: ViewParams, bundle?: string) {
        // 加载界面资源超时提示
        const timerId = setTimeout(this.onLoadingTimeoutGui, 10000);
        ...
        if (vp && vp.node) {
            ...
            bundle = bundle || resLoader.defaultBundleName;
            const res = await resLoader.loadAsync(bundle, vp.config.path, Prefab);
            if (res) {
                ...
                console.warn(`路径为【${vp.config.path}】的预制加载失败`);
                this.failure(vp);
            }
        }
    ...
    protected onCloseWindow(vp: ViewParams) {
        this.ui_nodes.delete(vp.config.path);
    }
    ...
    protected async showUi(vp: ViewParams): Promise<boolean> {
        ...
        if (r) {
            ...
            console.warn(`路径为【${vp.config.path}】的自定义预处理逻辑异常.检查预制上绑定的组件中 onAdded 方法,返回true才能正确完成窗口显示流程`);
            this.failure(vp);
        }
        return r;
    }
    ...
    remove(prefabPath: string, isDestroy?: boolean): void {
        ...
        if (vp) {
            ...
            // 不释放界面，缓存起来待下次使用
            if (release === false) {
                this.ui_cache.set(vp.config.path, vp);
            }
    ...
    clear(isDestroy: boolean): void {
        ...
        this.ui_nodes.forEach((value: ViewParams, key: string) => {
            this.remove(value.config.path, isDestroy);
            value.valid = false;
        });
        this.ui_nodes.clear();
